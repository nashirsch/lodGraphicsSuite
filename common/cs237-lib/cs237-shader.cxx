...
__detail::Shader::Shader (GLenum kind)
  : _src("<uninitialized>"),
    _obj(std::make_shared<__detail::Shader::Obj>(glCreateShader(kind))),
    _kind(kind),
    _initialized(false)
{
    if (this->Id() == 0) {
        std::cerr << "Error creating shader: kind = " << ShaderName(kind)
...
bool __detail::Shader::LoadFromFile (const char *file)
{
    ...
    if (this->_initialized) {
      // shader has already been initialized
        return false;
    }
...
bool __detail::Shader::LoadFromString (const char **src)
{
    ...
    if ((this->_initialized) || (src == nullptr)) {
      // shader has already been initialized or no input
        return false;
    }
...
// create a shader program from a vertex and fragment shaders
//
    ...
    : _obj(std::make_shared<ShaderProgram::Obj>(glCreateProgram ()))
{
    if (this->Id() == 0) {
        ...
        std::cerr << "  Vertex shader source: " << vsh.Source() << "\n";
        std::cerr << "  Fragment shader source: " << fsh.Source() << "\n";
        exit (1);
    }
...
// create a shader program from a vertex, geometry, and fragment shaders
//
    ...
    : _obj(std::make_shared<ShaderProgram::Obj>(glCreateProgram ()))
{
    if (this->Id() == 0) {
        std::cerr << "Error creating shader program\n" << std::endl;
        std::cerr << "  Vertex shader source: " << vsh.Source() << "\n";
        std::cerr << "  Geometry shader source: " << gsh.Source() << "\n";
        std::cerr << "  Fragment shader source: " << fsh.Source() << "\n";
        exit (1);
    }
...
// delete the underlying shader program object
//
...
// return the location of an attribute variable in this shader
//
...
// return the location of a uniform variable in this shader
//
GLint ShaderProgram::UniformLocation (const char *name)
{
    GLint id = glGetUniformLocation (this->Id(), name);
    if (id < 0) {
        std::cerr << "Error: shader uniform variable \"" << name << "\" is invalid\n";
        exit (1);
    }
    return id;
}

// return the index of a uniform block in this shader
//
GLint ShaderProgram::UniformBlockIndex (const char *name)
{
    GLint id = glGetUniformBlockIndex (this->Id(), name);
    if (id < 0) {
        std::cerr << "Error: shader uniform block \"" << name
            << "\" is invalid\n";
        exit (1);
    }
    return id;
