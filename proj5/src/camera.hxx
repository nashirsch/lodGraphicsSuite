...
class Camera {
  ...
  //! translate the camera position
    void lateral(double step);
    void longitudinal(double step);
  ...
  //! \return the screen-space error
    ...
    float HalfFOV() { return(this->_halfFOV); }
...
// Frustum class to store defining information of the 6 planes
class Frustum {
  public:
    ...
    cs237::vec3d normals[6];
    double distances[6]; // distance from a point on the plane to the origin
